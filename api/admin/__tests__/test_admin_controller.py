...
from api.admin import Admin, post_handler
...
async def test_post_handler(*args):
    with patch('json.loads') as loads_mock:
        with async_patch.object(Admin, 'create') as create_mock:
            with patch.object(Admin, 'make_dto') as make_dto_mock:
                with patch.object(Error, 'handle') as handle_mock:
                    with patch.object(DB, 'get_provider') as get_provider_mock:
                        ...
                        make_dto_mock.return_value = None
                        get_provider_mock.return_value = CoroutineMock()

                        res = await post_handler(mock_req)
                        
                        mock_req.text.assert_called()
                        loads_mock.assert_called()
                        make_dto_mock.assert_called()
                        get_provider_mock.assert_called_with(mock_req)
    ...
    #         with patch.object(DB, 'get_provider') as get_mock:
    #             with patch('json.loads') as loads_mock:
    #                 with patch.object(Admin, 'update') as update_mock:
    #                     with patch.object(Error, 'handle') as handle_mock:
...
#                 with patch.object(Admin, 'remove') as remove_mock:
#                     with patch.object(Error, 'handle') as handle_mock:
#                         mock_req = MagicMock()
