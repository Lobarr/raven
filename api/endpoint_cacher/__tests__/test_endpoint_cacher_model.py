...
class TestEndpointCacher:
    ...
    async def test__set_indexes(self, *args):
        ...
        for call in mock_hset.await_args_list:
            ...
            expect([mock_ctx['service_id'], mock_ctx['path']]).to(
                contain(call[0][2]))
    ...
    async def test__search_indexes(self, *args):
        ...
        mock_hscan.return_value = (
            None, [(expected_id.encode('utf-8'), mock_search.encode('utf-8'))])
    ...
    async def test_get_by_id(self, *args):
        ...
        expect(cache).to(equal(pydash.merge(expected_cache, {
    ...
    async def test_get_by_service_id(self, *args):
        with asynctest.patch.object(EndpointCacher, '_search_indexes') as _search_index_mock:
            ...
            _search_index_mock.assert_awaited_with(
                endpoint_cache_service_id_index, mock_service_id, mock_db)
            mock_hgetall.assert_awaited()
            expect(mock_hgetall.await_args[0][0]).to(equal(mock_keys[0]))
            mock_smembers.assert_awaited()
            expect(mock_smembers.await_args[0][0]).to(equal(response_codes_id))
            expect(caches).to(have_len(1))
            expect(caches[0]).to(equal(pydash.merge(expected_cache, {
                'response_codes': expected_response_codes})))
