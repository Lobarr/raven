...
async def handle_circuit_breaker(breaker: dict, service: dict, request: web.Request, req: dict):
    ...
    if not breaker_count:
        await CircuitBreaker.set_count(
            str(breaker['_id']), 0,
            breaker['period'], DB.get_redis(request)
        )
    ...
    await Async.all([
        CircuitBreaker.incr_tripped_count(
            str(breaker['_id']), 
...
async def handle_service(service: dict, remote: str):
    ...
    if (
        not pydash.is_empty(service['whitelisted_hosts']) 
        and remote not in service['whitelisted_hosts'] 
        or not pydash.is_empty(service['blacklisted_hosts']) 
        and remote in service['blacklisted_hosts']
    ):
        raise Exception({
...
async def handle_request(request: web.Request, service: dict, endpoint_cacher: dict):
    req_ctx = {
...
async def handle_insights(request: web.Request, response: dict, service_id: str, elapsed_time: int, cache: bool):
    ctx = {
        'method': request.method,
...
async def handle_rate_limiter(request: web.Request, service_id: str, rule: dict):
...
async def handle_request_validator(validator: dict, ctx: str, method: str):
    if method == validator['method']:
        'schema' in validator and await RequestValidator.validate_schema(ctx, validator['schema'])

        if (
            'password_field' in validator 
            and 'password_policy' in validator 
            and validator['password_field'] in ctx
        ):
            await RequestValidator.enforce_policy(ctx[validator['password_field']], validator['password_policy'])

            if 'strength' in validator['password_policy']:
                await RequestValidator.enforce_strength(ctx[validator['password_field']], validator['password_policy']['strength'])


@web.middleware
async def proxy(request: web.Request, handler):
    try:
        req_start_time = time()
        ...
        if (
            not pydash.is_empty(breaker) 
            and breaker['status'] == CircuitBreakerStatus.ON.name
        ):
            if req['status'] in breaker['status_codes']:
                checks.append(
                    handle_circuit_breaker(breaker, service, request, req)
                )
        ...
        checks.append(handle_insights(
            request, req, str(service['_id']), int(req_elapsed_time), req_cache_hit)
        )

        await Async.all(checks)

        return web.Response(
            body=Bytes.decode_bytes(req['body_bytes']),
            status=req['status'],
