...
from api.insights.controller import post_handler, get_handler, table, put_handler, delete_handler
    ...
    with patch.object(Validate, 'object_id') as object_id_mock:
      with patch.object(DB, 'get') as get_mock:
        with patch.object(Insights, 'get_all') as get_all_mock:
          with patch.object(Insights, 'get_by_id') as get_by_id_mock:
                      ...
                      with patch.object(Insights, 'get_by_method') as get_by_method_mock:
                        with patch.object(Error, 'handle') as handle_mock:
                          with patch.object(Bson, 'to_json') as to_json_mock:
                            ...
                            expect(get_by_remote_ip_mock.call_args[0][0]).to(equal(mock_query['remote_ip']))
                            get_mock.assert_called()
                            ...
                            expect(get_by_status_code_mock.call_args[0][0]).to(equal(mock_query['status_code']))
                            get_mock.assert_called()
                            ...
                            expect(get_by_scheme_mock.call_args[0][0]).to(equal(mock_query['scheme']))
                            get_mock.assert_called()
