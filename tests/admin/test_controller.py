  ...
  async def test_post_handler(self, *args):
    with patch.object(DB, 'get') as get_mock:
      with patch('json.loads') as loads_mock:
        with patch.object(Admin, 'create') as create_mock:
              ...
              with patch.object(Admin, 'get_by_username') as get_by_username_mock:
                with patch.object(Error, 'handle') as handle_mock:
                  with patch.object(Bson, 'to_json') as to_json_mock:
                    ...
                    expect(get_by_email_mock.call_args[0][0]).to(equal(mock_query['email']))
                    get_mock.assert_called()
                    ...
                    expect(get_by_username_mock.call_args[0][0]).to(equal(mock_query['username']))
                    get_mock.assert_called()
