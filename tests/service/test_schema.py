...
def test_service_schema():
  expect(service_schema).to(have_keys('_id', 'name', 'state', 'secure', 'targets', 'cur_target_index', 'whitelisted_hosts', 'blacklisted_hosts', 'public_key'))
  for prop in ['_id', 'name', 'state', 'public_key']:
